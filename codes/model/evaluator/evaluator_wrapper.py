    ...
    def __init__(
        ...
        if 't5' in cfg.text_embedder.version:
            ...
            #   local_files_only=False,
                ...
                use_fast=False,  # 关键修复：禁用 fast tokenizer，避免 tiktoken 解析错误
                local_files_only=True  # 强制使用本地文件，避免联网下载时缓存冲突
                )
        ...
        # model_path = pjoin("../evaluator/eval_klde-5_late-5_nlayer6_norm/model/net_best_top1.tar")
        model_path = cfg.inference.get("model_path", pjoin(cfg.exp.root_ckpt_dir, cfg.data.name, "evaluator", cfg.exp.name, "model", f"{model_name}.tar"))
